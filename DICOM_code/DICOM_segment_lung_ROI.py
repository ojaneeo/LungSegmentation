...
def segment_patient_lungs(patient_lungs):
    ...
    for img_file in file_list:
        ...
        for i in range(
                len(imgs_to_process)):  # we assume len is the number of slices (3) i is a slice each image has 3 slices
    ...
    for fname in file_list:
        ...
        imgs_to_process = np.load(
            fname.replace("lungmask", "images"))  # replace the WHOLE image with just the lungs masked out
        masks = np.load(fname)
        ...
        np.save(working_path + "ROI_" + patientID + ".npy", final_patient_images)
